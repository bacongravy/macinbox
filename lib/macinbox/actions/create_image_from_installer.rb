...
module Macinbox
  ...
  module Actions
    ...
    class CreateImageFromInstaller
      ...
      def initialize(opts)
        @installer_app     = opts[:installer_path]  or bail "Installer app not specified."
        @output_path       = opts[:image_path]      or bail "Output path not specified."
        @vmware_fusion_app = opts[:vmware_path]     or bail "VMWare Fusion app not specified."

        @disk_size         = opts[:disk_size]       or bail "Disk size not specified."
        @short_name        = opts[:short_name]      or bail "Short name not specified."
        @full_name         = opts[:full_name]       or bail "Full name not specified."
        @password          = opts[:password]        or bail "Password not specified."
        @auto_login        = opts[:auto_login]      or bail "Auto login not specified."
        @skip_mini_buddy   = opts[:skip_mini_buddy] or bail "Skip mini buddy not specified."
        @hidpi             = opts[:hidpi]           or bail "HiDPI not specified."

        @collector         = opts[:collector]       or bail "Collector not specified."
        @debug             = opts[:debug] || false

        Logger.bail "Installer app not found."     unless File.exist? @installer_app
        Logger.bail "VMware Fusion app not found." unless File.exist? @vmware_fusion_app
      end
      ...
      def check_macos_versions
        ...
        @install_info_plist = "#{@installer_app}/Contents/SharedSupport/InstallInfo.plist"
      	Logger.bail "InstallInfo.plist not found in installer app bundle." unless File.exist? @install_info_plist
        ...
        if installer_os_version_major != host_os_version_major || installer_os_version_minor != host_os_version_minor
        	Logger.bail "Host OS version (#{host_os_version}) and installer OS version (#{installer_os_version}) do not match."
        end
      end
